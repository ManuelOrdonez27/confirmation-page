function handleSubmit(e) {
  ...
  const sfxClick = document.getElementById('sfx-click');   // NEW
  const sfxIntro = document.getElementById('sfx-intro');   // NEW
...
window.addEventListener("load", () => {
  ...
  if (intro) playSound(intro, { volume: 0.20 });
});

// Click sound for text field
document.getElementById("name").addEventListener("focus", () => {
  playSound(sfxHover, { volume: 0.15 });
});

// Click sound for number field
